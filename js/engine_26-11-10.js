...
function draggable(piece, color){
    $(".wrapper>div>div." + piece +"." + color).draggable({
        ...
        start: function(){
            ...
            ps = getAllowedPositions($(this).attr('id'), $(this).attr('color'), $(this).attr('piece'));
            if(ps!='' || ps!= '#'){
...
function getBishopMoves(row, column, columns, color){
    ...
    for(i=column; i<8; i++){
        ...
        --r;
        if(r<1)break;
        if($('#' + columns[col] + r).hasClass(color))
            break;
        else if($('#' + columns[col] + r).hasClass(opColor))
        {
            pos += (columns[col]) + (r) + " ";
            break;
        }
...
function getKingMoves(row, column, columns, color){
    ...
    // alert(checkForCheck(columns[cols[0]] + rows[0], color));
        if(!(rows[0] < 1 || cols[0] < 1 || rows[0] > 8 || cols[0] > 8 || $('#' + columns[cols[0]] + rows[0]).hasClass(color))){
        ...
        if(!(rows[1] < 1 || cols[1] < 1 || rows[1] > 8 || cols[1] > 8 || $('#' + columns[cols[1]] + rows[1]).hasClass(color))){
        ...
        if(!(rows[2] < 1 || cols[2] < 1 || rows[2] > 8 || cols[2] > 8 || $('#' + columns[cols[2]] + rows[2]).hasClass(color))){
        ...
        if(!(rows[3] < 1 || cols[3] < 1 || rows[3] > 8 || cols[3] > 8 || $('#' + columns[cols[3]] + rows[3]).hasClass(color))){
        ...
        if(!(rows[4] < 1 || cols[4] < 1 || rows[4] > 8 || cols[4] > 8 || $('#' + columns[cols[4]] + rows[4]).hasClass(color))){
        ...
        if(!(rows[5] < 1 || cols[5] < 1 || rows[5] > 8 || cols[5] > 8 || $('#' + columns[cols[5]] + rows[5]).hasClass(color))){
        ...
        if(!(rows[6] < 1 || cols[6] < 1 || rows[6] > 8 || cols[6] > 8 || $('#' + columns[cols[6]] + rows[6]).hasClass(color))){
        ...
        if(!(rows[7] < 1 || cols[7] < 1 || rows[7] > 8 || cols[7] > 8 || $('#' + columns[cols[7]] + rows[7]).hasClass(color))){
...
function getPawnMoves(row, column, columns, color){
    ...
    if(column!=1 && $('#' + columns[column-1] + (rowS)).css('background-image') != "none" && $('#' + columns[column-1] + (rowS)).hasClass(opColor)){
        allowedPositions += columns[column-1] + (rowS) + " ";
    }
    if(column!=8 && $('#' + columns[column+1] + (rowS)).css('background-image') != "none" && $('#' + columns[column+1] + (rowS)).hasClass(opColor)){
        allowedPositions += columns[column+1] + (rowS) + " ";
    }
...
function getKnightMoves(row, column, columns, color){
    ...
    cols[7] = column +1;
    for(i=0; i<8; i++){
        if(!(rows[i] < 1 || cols[i] < 1 || rows[i] > 8 || cols[i] > 8 || $('#' + columns[cols[i]] + rows[i]).hasClass(color)))
...
function playSounds(src){
    ...
    if(canPlayAudio){
        $('body').append($('<audio id="sound_player">').attr('src', src));
...
function move(from, to){    
    ...
    $('#systemMessage').html( color.toUpperCase() + ' waiting');
    if(to.attr('piece')){
    ...
    to.attr('color' , color).attr('piece', piece).removeClass();
