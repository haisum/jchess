...
bobj.crv.ReportPage = {
    ...
    dispose : function() {
        ...
        var cssLayer = getLayer (this.id + '_stylesheet');
        if (cssLayer) {
            MochiKit.DOM.removeElement (this.id + '_stylesheet');
        }
    },
    ...
    update : function(update) {
        if (update && update.cons == "bobj.crv.newReportPage") {
            ...
            this.content = update.args.content;
        }
    },
    ...
    updateHTML : function(content) {
        ...
        if (content) {
            ...
            if (divs && divs[0]) {
                ...
                bobj.displayElementWithAnimation(divs[0]); //sets opacity to 0 before displaying element
