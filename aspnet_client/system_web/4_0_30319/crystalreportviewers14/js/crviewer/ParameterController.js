...
bobj.crv.params.ParameterController.prototype = {
    ...
    setParameters: function(paramList) {
        ...
        for (var i = 0; i < paramList.length; ++i) {
            ...
            var addParameter = function(panel, param, paramWidget, openAdvDialogCB, isValueOptional, panelCanChangeValues, clearValuesCB) {
                return function() {
            ...
            setTimeout(addParameter(this._panel, param, paramWidget, canOpenAdvDialog ? openAdvDialogCB : null, isValueOptional, panelCanChangeValues, clearValuesCB), TIMEER_INTERVAL * i);
        }
        
        if(paramList) {
            //Parameter panel has to be resized in case scroll bars appear
            var resizePanel = function (panel) { return function() { panel.resize(); };};
            setTimeout(resizePanel(this._panel), TIMEER_INTERVAL * paramList.length );
        }
