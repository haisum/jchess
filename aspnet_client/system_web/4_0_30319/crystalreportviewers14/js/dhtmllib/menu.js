...
// Returns       [MenuWidget] the new instance
{
	...
	o.addItemNumbering=MenuWidget_addItemNumbering
...
// Return   [MenuItem] the new item
{
	...
	var ret=o.items[i]=newMenuItem(o,id,text,cb,i+1,icon,dx,dy,disabled,disDx,disDy,false,alt)
	ret.menuIndex=i
	ret.dynHTML()
...
{
	var o=this,i=o.items.length
	var ret=o.items[i]=newMenuItem(o,id,text,cb,i+1,icon,dx,dy,disabled,disDx,disDy,true,alt)
	ret.menuIndex=i
	ret.dynHTML()
...
{	
	var o=this,i=o.items.length
	var item = newMenuItem(o,id,text,cb,i+1,icon,dx,dy,disabled,disDx,disDy,false,alt);			
...
{	
	var o=this,i=o.items.length
	var item = newMenuItem(o,id,text,cb,i+1,icon,dx,dy,disabled,disDx,disDy,true,alt);
	arrayAdd(o,'items',item,index);
	o.updateIndex();
...
function MenuWidget_addItemNumbering()
// add item numbering to the menu item tooltips
{
	var o=this,items=o.items,len=items.length
	for (i=0;i<len;i++)
		items[i].textOnlyLayer.title+=" - "+items[i].itemNo+_of+o.getTotalNumItems();
}
...
// Return [void]
{
	if (!this.hasNoLayer)
	{
		...
		if (!o.isSeparator)
		{
			...
			{
				o.textOnlyLayer.title=o.textOnlyLayer.innerText
...
function MenuItem_getHTMLPart(part)
{
	...
	switch(part)
	{
		...
		case 1: // the text
			return '<span id="'+(o.par.id+'_span_text_'+o.id)+'" tabIndex="0" role="menuitem">'+convStr(o.text)+'</span>'
		case 2:
